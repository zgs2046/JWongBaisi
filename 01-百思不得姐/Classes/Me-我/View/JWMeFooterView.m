...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        [[AFHTTPSessionManager manager] GET:@"http://api.budejie.com/api/api_open.php" parameters:params success:^(NSURLSessionDataTask *task, NSDictionary *responseObject) {
            NSArray *sqaures = [JWSquare objectArrayWithKeyValuesArray:responseObject[@"square_list"]];
...
- (void)createSquares:(NSArray *)sqaures
{
    ...
    // 宽度和高度
    CGFloat buttonW = JWScreenW / maxCols;
    CGFloat buttonH = buttonW;
    
    for (int i = 0; i<sqaures.count; i++) {
        // 创建按钮
        JWSqaureButton *button = [JWSqaureButton buttonWithType:UIButtonTypeCustom];
        // 监听点击
...
- (void)buttonClick:(JWSqaureButton *)button
{
